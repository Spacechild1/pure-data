...
static inline void dsptaskqueue_decrement(t_dsptaskqueue *x)
{
#ifdef MSVC_INTERLOCKED
    int remaining = _InterlockedDecrement(&x->dq_remaining); /* returns new value! */
#else
    int remaining = atomic_fetch_sub(&x->dq_remaining, 1) - 1;
#endif
#ifdef DEBUG_DSPTHREADS
...
{
    dsptaskqueue_free(x->dt_queue); /* release */
...
/* used instead of dsptask_sched() when a canvas is switched off */
void dsptask_skip(t_dsptask *x)
{
#ifdef DEBUG_DSPTHREADS
    fprintf(stderr, "queue %p: skip task %p\n", x->dt_queue, x);
#endif
    dsptaskqueue_decrement(x->dt_queue);
}
