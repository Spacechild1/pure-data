...
{
    if (garrayref_check(&x->x_ref))
        garray_redraw(x->x_ref.ar_garray);
}
...
static void *tabwrite_tilde_new(t_symbol *s)
{
    ...
    x->x_arrayname = s;
    garrayref_init(&x->x_ref);
    x->x_clock = clock_new(x, (t_method)tabwrite_tilde_redraw);
    x->x_f = 0;
    return (x);
}
...
static t_int *tabwrite_tilde_perform(t_int *w)
{
    ...
    if (garrayref_write_lock(&x->x_ref, &endphase, &vec))
    {
        if (endphase > phase)
        {
        ...
        garrayref_write_unlock(&x->x_ref);
    }
    return (w+4);
}
...
{
    x->x_arrayname = s;
    garrayref_set(&x->x_ref, s, &x->x_obj);
}
...
static void tabwrite_tilde_setup(void)
{
    tabwrite_tilde_class = class_new(gensym("tabwrite~"),
        ...
        sizeof(t_tabwrite_tilde), CLASS_DEFAULT, A_DEFSYM, 0);
...
static t_int *tabplay_tilde_perform(t_int *w)
{
    ...
    t_word *vec, *wp;
    int n = (int)(w[3]), phase = x->x_phase, endphase, npoints, nxfer, n3;
    if (!garrayref_read_lock(&x->x_ref, &npoints, &vec))
        goto zero;
    endphase = npoints < x->x_limit ? npoints : x->x_limit;
    if (phase >= endphase)
    {
        garrayref_read_unlock(&x->x_ref); /* ! */
        goto zero;
    }
...
{
    x->x_arrayname = s;
    garrayref_set(&x->x_ref, x->x_arrayname, &x->x_obj);
}
...
static void tabplay_tilde_setup(void)
{
    tabplay_tilde_class = class_new(gensym("tabplay~"),
        ...
        sizeof(t_tabplay_tilde), CLASS_DEFAULT, A_DEFSYM, 0);
...
static void tabread_tilde_setup(void)
{
    tabread_tilde_class = class_new(gensym("tabread~"),
        ...
        sizeof(t_tabread_tilde), CLASS_DEFAULT, A_DEFSYM, 0);
...
static t_int *tabread4_tilde_perform(t_int *w)
{
    ...
    t_sample *in = (t_sample *)(w[2]);
    t_sample *out = (t_sample *)(w[3]);
    int n = (int)(w[4]), maxindex, npoints, i;
...
static void tabread4_tilde_setup(void)
{
    tabread4_tilde_class = class_new(gensym("tabread4~"),
        ...
        sizeof(t_tabread4_tilde), CLASS_DEFAULT, A_DEFSYM, 0);
...
static t_int *tabosc4_tilde_perform(t_int *w)
{
    ...
    t_sample *in = (t_sample *)(w[2]);
    t_sample *out = (t_sample *)(w[3]);
    int n = (int)(w[4]), tabsize, mask, normhipart;
    union tabfudge tf;
    t_word *vec, *addr;
    t_float fnpoints, conv;
    double dphase;
    if (!garrayref_read_lock(&x->x_ref, &tabsize, &vec))
        goto zero;

    if (tabsize != x->x_lasttabsize)
    {
        /* check table size */
        int npoints = tabsize - 3;
        ...
        {
            pd_error(x, "tabosc4~: %s: number of points (%d) not a power of 2 plus three",
                x->x_arrayname->s_name, tabsize);
            x->x_fnpoints = -1; /* sentinel */
        }
        x->x_lasttabsize = tabsize;
    }

    if (x->x_fnpoints < 0) /* bad size */
    {
        garrayref_read_unlock(&x->x_ref); /* ! */
        goto zero;
    }
    fnpoints = x->x_fnpoints;
    mask = fnpoints - 1;
    conv = fnpoints * x->x_conv;
    dphase = fnpoints * x->x_phase + UNITBIT32;
    ...
    while (n--)
    {
        ...
        tf.tf_d = dphase;
        dphase += *in++ * conv;
        addr = vec + (tf.tf_i[HIOFFSET] & mask);
...
static void tabosc4_tilde_setup(void)
{
    tabosc4_tilde_class = class_new(gensym("tabosc4~"),
        ...
        sizeof(t_tabosc4_tilde), CLASS_DEFAULT, A_DEFSYM, 0);
...
static t_int *tabsend_perform(t_int *w)
{
    ...
    t_sample *in = (t_sample *)(w[2]);
    int n = (int)w[3], npoints;
    t_word *dest;
    if (garrayref_write_lock(&x->x_ref, &npoints, &dest))
    {
        if (n > npoints)
            n = npoints;
        while (n--)
        ...
        {
            clock_delay(x->x_clock, 0);
            x->x_graphcount = x->x_graphperiod;
        }
        garrayref_write_unlock(&x->x_ref);
    }
    return (w+4);
...
static void tabsend_setup(void)
{
    tabsend_class = class_new(gensym("tabsend~"), (t_newmethod)tabsend_new,
        (t_method)tabsend_free, sizeof(t_tabsend), CLASS_DEFAULT, A_DEFSYM, 0);
...
static t_int *tabreceive_perform(t_int *w)
{
    ...
    int n = (int)w[3], npoints;
    t_word *from;
    if (garrayref_read_lock(&x->x_ref, &npoints, &from))
    {
        int vecsize = npoints;
        if (vecsize > n)
            vecsize = n;
...
static void tabreceive_setup(void)
{
    tabreceive_class = class_new(gensym("tabreceive~"),
        ...
        sizeof(t_tabreceive), CLASS_DEFAULT, A_DEFSYM, 0);
...
static void tabread_float(t_tabread *x, t_float f)
{
    int npoints;
    t_word *vec;
    if (garrayref_get(&x->x_ref, &npoints, &vec, x->x_arrayname, &x->x_obj))
    {
        int n = f;
        if (n < 0) n = 0;
...
static void tabwrite_float(t_tabwrite *x, t_float f)
{
    int vecsize;
    t_word *vec;
    if (garrayref_get(&x->x_ref,
        &vecsize, &vec, x->x_arrayname, &x->x_obj))
    {
        int n = x->x_ft1;
        if (n < 0)
            n = 0;
